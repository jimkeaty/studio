...
import { CheckCircle2, AlertTriangle, XCircle, RefreshCw, Upload, Eye } from 'lucide-react';
  ...
  const runPreview = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/bulk-update-transactions', {
        ...
        body: JSON.stringify({ rows: SPREADSHEET_ROWS, mode: 'preview', year: 2025 }),
      });
  ...
  const runApply = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/bulk-update-transactions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ rows: SPREADSHEET_ROWS, mode: 'apply', year: 2025 }),
      });
    ...
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Fuzzy-match 75 spreadsheet rows against 2025 SmartBroker transactions and update sale/list prices.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Pre-loaded from the 2025 closed transactions spreadsheet (75 rows)
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-4 text-sm">
            <div className="rounded-md border px-3 py-2">
              <div className="font-semibold text-lg">{SPREADSHEET_ROWS.length}</div>
              <div className="text-muted-foreground">Total Rows</div>
            </div>
            <div className="rounded-md border px-3 py-2">
              <div className="font-semibold text-lg">2025</div>
              <div className="text-muted-foreground">Year</div>
            </div>
            <div className="rounded-md border px-3 py-2">
              <div className="font-semibold text-lg">
                {SPREADSHEET_ROWS.filter(r => r.salePrice != null).length}
              </div>
              <div className="text-muted-foreground">Rows with Sale Price</div>
            </div>
          </div>
          <div className="mt-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="flex gap-3 mb-4 text-sm">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={showNoMatch}
                    onChange={e => setShowNoMatch(e.target.checked)}
                    className="rounded"
                  />
                  Show No-Match rows
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={showNoChanges}
                    onChange={e => setShowNoChanges(e.target.checked)}
                    className="rounded"
                  />
              ...
              <p className="text-sm text-muted-foreground mb-4">
                ...
                {!showNoMatch && ` (${preview.noMatches} no-match rows hidden)`}
                {!showNoChanges && ` (${results.filter(r => Object.keys(r.changes).length === 0 && r.status !== 'no_match').length} unchanged rows hidden)`}
              </p>
              ...
              {preview.withChanges > 0 && (
                ...
                >
                  ...
                  {applying
                    ? 'Applying Updates…'
                    : `Apply ${preview.withChanges} Update${preview.withChanges !== 1 ? 's' : ''} to Firestore`}
                </Button>
              )}
              {preview.withChanges === 0 && (
                <Alert>
                  ...
                  <AlertDescription>
                    All matched transactions already have the correct sale/list prices.
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
