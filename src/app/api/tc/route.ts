...
export async function POST(req: NextRequest) {
  try {
    ...
    const txDoc: Record<string, any> = {
      ...
      manualCommissionPercentOverride: toBool(body.manualCommissionPercentOverride),
      manualCommissionPercentOverriddenBy: toStr(body.manualCommissionPercentOverriddenBy) || null,
      manualCommissionPercentOverriddenAt: toStr(body.manualCommissionPercentOverriddenAt) || null,
    ...
    (async () => {
      try {
        ...
        if (warrantyEducationSides.length > 0) {
          const invitation = await sendAphwEducationInvitations(txDoc, warrantyEducationSides.map(side => side.toLowerCase() as 'buyer' | 'seller'));
