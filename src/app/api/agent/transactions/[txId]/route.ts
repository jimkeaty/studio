...
const AGENT_ALLOWED_FIELDS = new Set([
  ...
  'appraisalConditioned', 'appraisalPeriodDays', 'depositDueDays', 'depositDeadline', 'financingCommitmentDays', 'dueDiligenceDays', 'titleCurativeDays', 'closingDays',
  // Financial
    ...
    void (async () => {
      try {
        ...
        if (warrantyEducationSides.length > 0) {
          const invitation = await sendAphwEducationInvitations({ ...txData, ...updates }, warrantyEducationSides.map(side => side.toLowerCase() as 'buyer' | 'seller'));
          ...
          const agentConfirmation = invitation.sentTo.length > 0
            ? `We sent the America’s Preferred Home Warranty consultation invitation for the ${warrantyEducationSides.join(' and ').toLowerCase()} on ${txAddress}. The office team will also follow up.`
            : `We received your America’s Preferred Home Warranty education request for the ${warrantyEducationSides.join(' and ').toLowerCase()} on ${txAddress}. The office team will contact the client directly because the invitation email could not be sent.`;
          await txRef.update({
