...
export function SignLocationPicker({ address, latitude, longitude, notes, onChange }: Props) {
  const mapRef = useRef<HTMLDivElement | null>(null); const instanceRef = useRef<any>(null); const markerRef = useRef<any>(null); const [search, setSearch] = useState(address); const [message, setMessage] = useState('');
  ...
  useEffect(() => {
    let cancelled = false; const start = async () => {
      ...
      if (!window.L) { const css = document.createElement('link'); css.rel = 'stylesheet'; css.href = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'; document.head.appendChild(css); await new Promise<void>((resolve, reject) => { const script = document.createElement('script'); script.src = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.js'; script.onload = () => resolve(); script.onerror = () => reject(new Error('Map could not load.')); document.head.appendChild(script); }); }
      if (cancelled || !window.L || !mapRef.current) return;
      const center = hasCoordinates ? [lat, lng] : [LAFAYETTE.lat, LAFAYETTE.lng]; const map = window.L.map(mapRef.current).setView(center, hasCoordinates ? 17 : 12); window.L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19, attribution: '© OpenStreetMap contributors' }).addTo(map); instanceRef.current = map;
      const setMarker = (point: any) => { if (markerRef.current) markerRef.current.remove(); markerRef.current = window.L.marker(point, { draggable: true }).addTo(map); markerRef.current.on('dragend', () => { const next = markerRef.current.getLatLng(); applyPin(next.lat, next.lng); }); };
  ...
  useEffect(() => { if (!instanceRef.current || !hasCoordinates || !window.L) return; const point = window.L.latLng(lat, lng); instanceRef.current.setView(point, 17); if (markerRef.current) markerRef.current.setLatLng(point); else markerRef.current = window.L.marker(point, { draggable: true }).addTo(instanceRef.current); }, [latitude, longitude]);
