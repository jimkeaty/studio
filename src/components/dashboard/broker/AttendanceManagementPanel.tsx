  ...
  return <>
    <Card className="border-2 border-violet-200 bg-gradient-to-b from-violet-50/70 to-background">
      ...
      <CardContent className="space-y-5 pt-5">
        ...
        <section aria-label="Monthly CGL attendance percentages" className="rounded-lg border border-violet-200 bg-violet-50/50 p-4">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-semibold text-violet-950">Monthly CGL Attendance</p><p className="text-xs text-violet-800">Agent-recorded attendance across eligible scheduled opportunities through {cglAttendance?.throughDate || '—'}.</p></div><p className="text-xs text-violet-800">Threshold not configured</p></div>
          <div className="mt-3 grid gap-3 md:grid-cols-3">{(['training', 'huddle', 'role_play_ids'] as const).map(type => {
            const metric = cglAttendance?.metrics.find(item => item.type === type);
            return <div key={type} className="rounded-md border border-violet-100 bg-background px-3 py-3"><p className="text-sm font-medium">{metric?.label || (type === 'role_play_ids' ? 'Role Play / New Agent IDS' : type === 'huddle' ? 'Team Huddle' : 'Optional Training')}</p>{metric?.percentage == null ? <><p className="mt-2 text-xl font-bold">No Data</p><p className="mt-1 text-xs text-muted-foreground">No eligible opportunities recorded.</p></> : <><p className="mt-2 text-xl font-bold">{metric.percentage}%</p><p className="mt-1 text-xs text-muted-foreground">{metric.numerator} attended / {metric.denominator} eligible opportunities · {metric.eligibleAgentCount} CGL agent{metric.eligibleAgentCount === 1 ? '' : 's'}</p></>}</div>;
          })}</div>
