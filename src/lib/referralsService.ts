...
export async function getFullDownline(
  ...
  // HOTFIX: The original implementation of this function performs queries
  // (e.g., fetching transaction data for other agents) that are not
  // permissible for client-side execution under existing security rules.
  // This causes "Missing or insufficient permissions" errors in the browser console.
  //
  // To resolve this, we are temporarily disabling the client-side database
  // queries and falling back directly to the mock data for development
  // environments. A proper fix involves creating a secure backend API
  // endpoint to provide this data.
  if (process.env.NODE_ENV === 'development') {
    return getMockFullDownline(agentId);
  }

  // In production, return an empty array to avoid errors and show a clean state.
  return [];
}
 ...
 */
export async function getAllBrokerageRecruits(
  db: Firestore
): Promise<DownlineMember[]> {
  try {
    ...
    const allReferrals = referralsSnap.docs.map(
      (d) => d.data() as AgentReferral
    );
    const allRecruitIds = allReferrals.map((r) => r.recruitedAgentId);

    const qualDocsSnap = await getDocs(
      query(
        collection(db, 'referral_qualifications'),
        where(documentId(), 'in', allRecruitIds)
      )
    );
    const qualMap = new Map<string, ReferralQualification>();
    qualDocsSnap.forEach((d) =>
      qualMap.set(d.id, d.data() as ReferralQualification)
    );
    ...
    const progressResults = await Promise.all(
      Array.from(qualMap.values()).map((q) =>
        computeQualificationProgress(db, q).catch((err) => {
          console.error(
            `Admin Console: Could not compute progress for ${q.recruitedAgentId}:`,
            err.message
          );
          return null;
        })
    ...
    allReferrals.forEach((r) =>
      referralMap.set(r.recruitedAgentId, r.referrerAgentId)
    );
