...
// ✅ Agent ID corrections: bad slug → canonical slug
const AGENT_ID_MAP = {
...
// ✅ Canonical display names (agentId -> correct name)
...
function addEvent(rollups, agentRaw, year, kind) {
  ...
  function normalizeStatus(raw) {
    ...
    // strip spreadsheet prefix codes like x-, y-, z-
    s = s.replace(/^[xyz]-/i, "");
...
async function writeStagingAgentYear({ agent, year, rollup }) {
  if (!CONFIG.writeToFirestore) {
    console.log("🟡 WRITE_TO_FIRESTORE is OFF. No writes will occur.");
    return;
  }
  if (!CONFIG.confirmWrite) {
    console.log("🛑 Safety stop: set CONFIRM_WRITE=YES to allow any Firestore write.");
    return;
  }
  ...
  console.log(`✅ STAGING WRITE OK: imports_staging/${docId}`);
}
...
async function writeToFirestore(preview) {
  ...
  (function debugDuplicateRollups() {
    ...
    if (!dupes.length) {
      console.log("✅ No duplicate rollups detected.");
      return;
    }

    console.log(`⚠️ Duplicate rollups detected: ${dupes.length}`);
    console.log(
      dupes
        .slice(0, 50)
        .map(([id, c]) => `  - ${id}  (x${c})`)
  ...
  if (!CONFIG.stagingOnly && CONFIG.allowProdUpsert) {
    if (!LIMIT_YEAR) {
      die("🛑 Safety stop: Prod upsert requires LIMIT_YEAR. Aborting.");
    }
  }
  ...
  if (CONFIG.stagingOnly) {
    console.log('\n✅ Staging-only mode: skipping prod upserts (agents + agentYearRollups).');
  ...
  if (!CONFIG.allowProdUpsert) {
    console.log('\n🛑 Prod upserts blocked. Set ALLOW_PROD_UPSERT=YES to enable prod writes.');
    return;
  }
  if (!CONFIG.confirmWrite) {
    console.log('\n🛑 Prod upserts blocked. Set CONFIRM_WRITE=YES to enable prod writes.');
    return;
  }
...
for (const [agentId, doc] of agentsEntries) {
  ...
  await commitBatch();
  console.log(`\n✅ Firestore prod upsert complete. Ops committed: ${committed}`);
}
